...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(char c:word.toCharArray()){
			if(c == 'a' || c == 'A' || 
				...
				if(!newSyll) {
					syllableCount++;
					if(syllableCount > 1 && i == word.length()-1 && (c == 'E' || c == 'e')){
						syllableCount--;
					}
	...
	public double getFleschScore()
	{
		...
		float numWords = (float)this.getNumWords();
		float numSentences = (float)this.getNumSentences();
		float numSyllables = (float)this.getNumSyllables();
